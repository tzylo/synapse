...
In relation to logging practices, it is important to note that the installation logger's directory path has recently been modified from `../../logs` to `../../../logs`. This adjustment shifts the installation logs from the source folder to a dedicated server folder, ensuring better organization and accessibility within the project hierarchy. While the logger initialization flow continues to verify the existence of the directory and create it if necessary, this change impacts how logs are stored and accessed, which may have implications for developers interacting with the logging system.
